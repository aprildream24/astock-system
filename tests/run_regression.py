...
SUITES = ["test_engines.py", "test_push_crypto.py", "test_guard_t1.py",
          ...
          "test_rehearsal.py",
          # 2026-09-21 新增：盘前/竞价/盘中持仓+自选操作建议（去弱留强）
          # + 模拟盘 3322/3331 分仓 + 盘中直接买入 + 自动换股回补。
          "test_advice_20260921.py",
          # 2026-09-21 第二批：持仓全生命周期（hold_days/到期阶段/裁决升级）
          # + 板块退潮检测（买入否决 + 持仓预警）。
          "test_lifecycle_20260921.py",
          # 2026-09-22：持仓体检一票一卡版式 + 换股候选排序即优先级
          # + 模拟盘成交即时 force 推送 + rec_picks 存综合可执行分。
          "test_layout_20260922.py",
          # 2026-09-25：Alpha 因子组（qlib 蒸馏）+ 唐奇安突破 + 历史候选
          # 并入盘中监控（「到达买点的票随时推，不要永远只是那几只」）。
          "test_alpha_20260925.py",
          # 2026-09-25 第二批：跨天确认（收盘首推→双确认→三确认）。
          "test_confirm_cross_day.py",
