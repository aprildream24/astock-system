...
## 推荐配额与板块热度（2026-09-18）

**用户口径**：行情好时，评分高的个股**全部推荐**（不再限制 3 个），并标注板块热度。

配额由 `scoring.market_heat(emo)` 一处决定（行情档位 → 推荐条数）：

| 行情档位（十维情绪分） | 推荐条数 | 同板块上限 | 连板席位 |
|---|---|---|---|
| 亢奋（≥76 且数据达标） | **不限**（全部符合条件标的） | 3 | 4 |
| 偏热（≥60 且数据达标） | **不限** | 2 | 3 |
| 均衡 / 偏冷 / 冰点 | 3（原纪律不变） | 1 | 2 |

- **数据未达标（覆盖不足）时绝不放开** —— 情绪分本身不可信，放量等于放大噪声。
- 非热点行情**不因本次改动收紧**（仍是 3 只），避免用户感知突变。
- 放开限量后主推送仍可读：前 2 只出完整卡（含买区条形图），其余走紧凑行；
  紧凑行总预算 14 条，超出部分在推送里明确写「另有 N 只见网页版完整详情」；
  最后还有一层「整卡回退」长度保险丝，保证正文不超 PushPlus 的 19000 上限
  （超了会被硬截断，切在半张卡中间，比少显示更糟）。

**板块热度**由 `pipeline/sector.py` 提供（东财行业板块榜 + 个股 `f100` 行业归属，
纯 HTTP、零鉴权、失败即降级为"不标注"）：

- 候选卡新增「板块热度」行：`半导体 +3.20% 🔥强 主力+12.3亿`（涨红跌绿，A股口径）
- 候选行带板块短标签；主推送顶部有「今日板块热度 · 领涨行业」TOP6
- 板块冷热**真实参与评分**（🔥强 ×1.03 / ❄弱 ×0.90），并做**同板块限额**
- ⚠️ 顺带修掉两处静默失效：`sector_temp` 冷热因子与「同板块去重」此前**都没有
  数据源**（前者恒为 None，加成从未生效；后者退化成"按池别去重"——不同行业的
  波段票被当成同一板块互斥，每次只活一只，是推荐数量偏少的隐藏原因之一）。

## 模拟盘自动运行（2026-09-18）

**10 万元起步，收盘后自动建仓 + 每日巡逻。**

| 项 | 值 |
|---|---|
| 初始资金 | ¥100,000（`executor.RISK["init_cash"]`） |
| 持仓上限 / 单日委托 | 4 只 / 6 笔 |
| 单票目标资金 | 净值 × min(70%÷4, 25%) ≈ 17.5%（整手 100 股取整） |
| 单笔金额区间 | ¥1,000 ~ ¥60,000 |
| 止损 | 硬止损 -3% / 波段 -6% / ATR 保护线（只收紧）/ MA20 破位 |
| 止盈 | 持仓浮盈 ≥15%（按持仓收益，不是日涨幅） |
| 熔断 | 日内组合亏损 -3% → 当日锁定不开新仓（卖出不受限） |
| T+1 | 按批次管理：昨日批次可卖、今日批次不可卖 |

- 建仓口径与推送**同源**：只买当日 `rec_picks` 里「现在买 / 等回踩 / 小仓试」
  **且现价确实落在买区内**的票 —— 推什么就模拟买什么（反之不追高）。
- 风控只有 `place_order()` 一个出口（涨跌停/资金/T+1/累计敞口/持仓上限/单日
  委托数全在那里），`auto_open()` 不重复判一遍，避免两套规则打架。
- **运行位置**：挂在 `stock.yml` 收盘构建（close）之后，与主链**共享同一份
  行情库缓存**。为什么不单开一个 workflow：① 单跑时 runner 是空库，无 K 线无
  持仓，什么都判不了；② 模拟盘状态与行情库同处 `cache/`，随主链一次 save
  持久化，不会出现"两个 workflow 各存一份、缓存 GC 把状态删回旧版本"；
  ③ 不必再增加一个会静默失效的定时器。手动通道见 `executor.yml`
  （restore-only，只读快照，不覆盖线上状态）。
- 推送：`mode=exec_auto`，头部带账户概览（净值 / 现金 / 持仓数 / 累计收益）。
- ⚠️ 修复的是**功能缺失**：原 `executor.py` 只有退出裁决、**没有任何买入路径**，
  账户永远空仓 ⇒ 巡逻无对象 ⇒ 日志为空 ⇒ 一条推送都发不出去。用户看到的
  "模拟盘没在跑"根因在此（叠加 executor workflow 从来没挂过定时器这条）。
